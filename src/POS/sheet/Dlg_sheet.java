...
import POS.sheet.S1_sheet.to_sheet;
 ...
 * @author Maytopacka
 */
public class Dlg_sheet extends javax.swing.JDialog {
...
//</editor-fold>
    ...
    private Dlg_sheet(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        setUndecorated(true);
        initComponents();
        myInit();
    ...
    private Dlg_sheet(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        setUndecorated(true);
        initComponents();
        myInit();
    ...
    private Dlg_sheet myRef;

    private void setThisRef(Dlg_sheet myRef) {
        this.myRef = myRef;
    }
    private static java.util.Map<Object, Dlg_sheet> dialogContainer = new java.util.HashMap();
    ...
    public static Dlg_sheet create(java.awt.Window parent, boolean modal) {
    ...
    public static Dlg_sheet create(java.awt.Window parent, java.awt.Dialog.ModalityType modalType) {

        if (parent instanceof java.awt.Frame) {

            Dlg_sheet dialog = dialogContainer.get(parent);

            if (dialog == null) {
                dialog = new Dlg_sheet((java.awt.Frame) parent, false);
        ...
        if (parent instanceof java.awt.Dialog) {
            ...
            if (dialog == null) {
                dialog = new Dlg_sheet((java.awt.Dialog) parent, false);
                dialog.setModalityType(modalType);
                dialogContainer.put(parent, dialog);
                java.util.logging.Logger.getAnonymousLogger().
                        log(Level.INFO, "instances: {0}", dialogContainer.size());
    ...
    public static void main(String args[]) {
        ...
        Dlg_sheet dialog = Dlg_sheet.create(new javax.swing.JFrame(), true);
    ...
    private void initComponents() {
        ...
        tbl_sheet = new org.jdesktop.swingx.JXTable();
        ...
        tbl_sheet.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
        ...
        tbl_sheet.setOpaque(false);
        tbl_sheet.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                tbl_sheetMouseClicked(evt);
            }
        });
        jScrollPane2.setViewportView(tbl_sheet);
        ...
        tf_no.setFont(new java.awt.Font("Tahoma", 1, 12)); // NOI18N
    ...
    private void tbl_sheetMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_sheetMouseClicked
        select_sheet();
    }//GEN-LAST:event_tbl_sheetMouseClicked
    ...
    private org.jdesktop.swingx.JXTable tbl_sheet;
...
//                btn_0.doClick();
    ...
    private ArrayListModel tbl_sheet_ALM;
    private TblsheetModel tbl_sheet_M;

    private void init_tbl_sheet() {
        tbl_sheet_ALM = new ArrayListModel();
        tbl_sheet_M = new TblsheetModel(tbl_sheet_ALM);
        tbl_sheet.getTableHeader().
                setPreferredSize(new Dimension(100, 40));
        tbl_sheet.setModel(tbl_sheet_M);
        tbl_sheet.setSelectionMode(ListSelectionModel.SINGLE_INTERVAL_SELECTION);
        tbl_sheet.setRowHeight(25);
        int[] tbl_widths_sheet = {100};
        for (int i = 0, n = tbl_widths_sheet.length; i < n; i++) {
            if (i == 0) {
                continue;
            }
            TableWidthUtilities.
                    setColumnWidth(tbl_sheet, i, tbl_widths_sheet[i]);
        }
        Dimension d = tbl_sheet.getTableHeader().
                getPreferredSize();
        d.height = 30;
        tbl_sheet.getTableHeader().
                setPreferredSize(d);
        ...
        tbl_sheet.setRowHeight(35);
        tbl_sheet.setFont(new java.awt.Font("Arial", 0, 12));
    }

    private void loadData_sheet(List<to_sheet> acc) {
        tbl_sheet_ALM.clear();
        tbl_sheet_ALM.addAll(acc);
    }

    public static class TblsheetModel extends AbstractTableAdapter {

        public static String[] COLUMNS = {
            "No"
        };

        public TblsheetModel(ListModel listmodel) {
        ...
        public Object getValueAt(int row, int col) {
            to_sheet tt = (to_sheet) getRow(row);
            switch (col) {
                case 0:
                    return tt.no;
                default:
    ...
    private void data_cols() {
        ...
        loadData_sheet(S1_sheet.ret_data(search));
    }

    private void add_sheet() {
        int id = -1;
        int no = FitIn.toInt(tf_no.getText());
        to_sheet to = new to_sheet(id, no);
        S1_sheet.add_sheet(to);
        data_cols();
        clear_sheet();
        Alert.set(1, "");
    }
    ...
    private void select_sheet() {
        int row = tbl_sheet.getSelectedRow();
        if (row < 0) {
            return;
        }
        to_sheet to = (to_sheet) tbl_sheet_ALM.get(tbl_sheet.
                convertRowIndexToModel(row));
        tf_no.setText(FitIn.fmt_woc(to.no));
    }

    private void edit_sheet() {
        int row = tbl_sheet.getSelectedRow();
        if (row < 0) {
            return;
        }
        ...
        int id = to.id;
        int no = FitIn.toInt(tf_no.getText());
        to_sheet to1 = new to_sheet(id, no);
        S1_sheet.edit_sheet(to1);
        data_cols();
        clear_sheet();
         Alert.set(2, "");
    }

    private void clear_sheet() {
        tf_no.setText("");
        tf_no.grabFocus();
    }
